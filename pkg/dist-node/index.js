...
var gestures = require('@polymer/polymer/lib/utils/gestures.js');
var resolveUrl_js = require('@polymer/polymer/lib/utils/resolve-url.js');
var renderStatus_js = require('@polymer/polymer/lib/utils/render-status.js');
var mutableDataBehavior_js = require('@polymer/polymer/lib/legacy/mutable-data-behavior.js');
var templatizerBehavior_js = require('@polymer/polymer/lib/legacy/templatizer-behavior.js');
...
const IronFitBehavior = {
  ...
  get _defaultPositionTarget() {
    var parent = polymer_dom_js.dom(this).parentNode;
...
const IronResizableBehavior = {
  ...
  _onIronRequestResizeNotifications: function (event) {
    var target =
    /** @type {!EventTarget} */
    polymer_dom_js.dom(event).rootTarget;
...
const IronFocusablesHelper = {
  ...
  _collectTabbableNodes: function (node, result) {
    ...
    if (element.localName === 'content' || element.localName === 'slot') {
      children = polymer_dom_js.dom(element).getDistributedNodes();
    } else {
      // Use shadow root if possible, will check for distributed nodes.
      children = polymer_dom_js.dom(element.root || element).children;
    }
...
`,
  ...
  prepare: function () {
    if (this.opened && !this.parentNode) {
      polymer_dom_js.dom(document.body).appendChild(this);
    }
  },
  ...
  complete: function () {
    if (!this.opened && this.parentNode === document.body) {
      polymer_dom_js.dom(this.parentNode).removeChild(this);
    }
  },
...
IronOverlayManagerClass.prototype = {
  ...
  get deepActiveElement() {
    ...
    while (active.root && polymer_dom_js.dom(active.root).activeElement) {
      active = polymer_dom_js.dom(active.root).activeElement;
    }
...
function _shouldPreventScrolling(event) {
  ...
  var target = polymer_dom_js.dom(event).rootTarget;

  if (event.type !== 'touchmove' && lastRootTarget !== target) {
    lastRootTarget = target;
    lastScrollableNodes = _getScrollableNodes(polymer_dom_js.dom(event).path);
...
const IronOverlayBehaviorImpl = {
   ...
   */
  get _focusNode() {
    return this._focusedChild || polymer_dom_js.dom(this).querySelector('[autofocus]') || this;
  },
  ...
  attached: function () {
    ...
    this._observer = polymer_dom_js.dom(this).observeNodes(this._onNodesChange);
  },
  detached: function () {
    polymer_dom_js.dom(this).unobserveNodes(this._observer);
    this._observer = null;
  ...
  _applyFocus: function () {
    if (this.opened) {
      ...
      if (this.restoreFocusOnClose && this.__restoreFocusNode) {
        ...
        if (activeElement === document.body || polymer_dom_js.dom(this).deepContains(activeElement)) {
          this.__restoreFocusNode.focus();
        }
      }
  ...
  _onCaptureFocus: function (event) {
    ...
    var path = polymer_dom_js.dom(event).path;
  ...
  __onCaptureScroll: function (event) {
    ...
    if (polymer_dom_js.dom(event).path.indexOf(this) >= 0) {
      return;
    }
...
`,
  ...
  get containedElement() {
    ...
    var nodes = polymer_dom_js.dom(this.$.content).getDistributedNodes();
...
const template = htmlTag_js.html`
<custom-style>
...
`,
  ...
  properties: {
     ...
     */
    _meta: {
      value: polymerLegacy_js.Base.create('iron-meta', {
        type: 'iconset'
      })
    }
  },
  ...
  _updateIcon: function () {
    if (this._usesIconset()) {
      if (this._img && this._img.parentNode) {
        polymer_dom_js.dom(this.root).removeChild(this._img);
      }
      ...
      polymer_dom_js.dom(this.root).appendChild(this._img);
    }
  }
});
...
polymerFn_js.Polymer({
  ...
  applyIcon: function (element, iconName) {
    ...
    if (svg) {
      ...
      var pde = polymer_dom_js.dom(element.root || element);
  ...
  removeIcon: function (element) {
    // Remove old svg element
    if (element._svgIcon) {
      polymer_dom_js.dom(element.root || element).removeChild(element._svgIcon);
  ...
  _createIconMap: function () {
    ...
    polymer_dom_js.dom(this).querySelectorAll('[id]').forEach(function (icon) {
      icons[icon.id] = icon;
    });
    return icons;
  },
...
const template$1 = htmlTag_js.html`<iron-iconset-svg name="icons" size="24">
<svg><defs>
...
const IronA11yAnnouncer = polymerFn_js.Polymer({
  _template: htmlTag_js.html`
    <style>
      :host {
...
`,
  ...
  attached: function () {
    ...
    this._observer = polymer_dom_js.dom(this).observeNodes(function (info) {
...
const IronLabel = polymerFn_js.Polymer({
  is: 'iron-label',
  listeners: {
    'tap': '_tapHandler'
  },
  ...
  _generateLabelId: function () {
    if (!this.id) {
      ...
      polymer_dom_js.dom(this).setAttribute('id', id);
    }
  },
  _findTarget: function () {
    if (this.for) {
      // external target
      var scope = polymer_dom_js.dom(this).getOwnerRoot();
      return polymer_dom_js.dom(scope).querySelector('#' + this.for);
    } else {
      // explicit internal target
      var el = polymer_dom_js.dom(this).querySelector('[iron-label-target]');

      if (!el) {
        // implicit internal target
        el = polymer_dom_js.dom(this).firstElementChild;
      }
  ...
  _tapHandler: function (ev) {
    ...
    var target = polymer_dom_js.dom(ev).localTarget;
  ...
  _applyLabelledBy: function () {
    if (this._forElement) {
      polymer_dom_js.dom(this._forElement).setAttribute('aria-labelledby', this.id);
    }
  },
  _forChanged: function () {
    if (this._forElement) {
      polymer_dom_js.dom(this._forElement).removeAttribute('aria-labelledby');
    }
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$2 = htmlTag_js.html`
<custom-style>
  <style is="custom-style">
    html {
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$3 = htmlTag_js.html`
...
const IronButtonStateImpl = {
  ...
  _spaceKeyDownHandler: function (event) {
    ...
    var target = polymer_dom_js.dom(keyboardEvent).localTarget; // Ignore the event if this is coming from a focused light child, since that
...
function Ripple(element) {
  ...
  polymer_dom_js.dom(this.waveContainer).appendChild(this.wave);
...
Ripple.prototype = {
  ...
  remove: function () {
    polymer_dom_js.dom(this.waveContainer.parentNode).removeChild(this.waveContainer);
  }
};
/**
...
`,
  ...
  attached: function () {
    ...
    if (this.parentNode.nodeType == 11) {
      ...
      this.keyEventTarget = polymer_dom_js.dom(this).getOwnerRoot().host;
    } else {
  ...
  addRipple: function () {
    ...
    polymer_dom_js.dom(this.$.waves).appendChild(ripple.waveContainer);
...
const PaperRippleBehavior = {
  ...
  ensureRipple: function (optTriggeringEvent) {
    if (!this.hasRipple()) {
      ...
      if (rippleContainer) {
        polymer_dom_js.dom(rippleContainer).appendChild(this._ripple);
      }
      ...
      if (optTriggeringEvent) {
        ...
        var domContainer = polymer_dom_js.dom(this._rippleContainer || this);
        var target = polymer_dom_js.dom(optTriggeringEvent).rootTarget;
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$4 = polymerLegacy_js.html`
...
`,
  ...
  _resolveSrc: function (testSrc) {
    var resolved = resolveUrl_js.resolveUrl(testSrc, this.$.baseURIAnchor.href); // NOTE: Use of `URL` was removed here because IE11 doesn't support
    // constructing it. If this ends up being problematic, we should
    // consider reverting and adding the URL polyfill as a dev dependency.
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$5 = htmlTag_js.html`
<custom-style>
  <style is="custom-style">
    html {
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$6 = htmlTag_js.html`
<custom-style>
  <style is="custom-style">
    html {
      /*
...
subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
const template$7 = htmlTag_js.html`<style>
  :host {
    display: inline-block;
    white-space: nowrap;
    cursor: pointer;
...
polymerFn_js.Polymer({
  ...
  attached: function () {
    ...
    renderStatus_js.afterNextRender(this, function () {
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$8 = htmlTag_js.html`<custom-style>
  <style is="custom-style">
    html {
...
const IronScrollTargetBehavior = {
  ...
  _scrollTargetChanged: function (scrollTarget, isAttached) {
    ...
    if (scrollTarget === 'document') {
      ...
      this.scrollTarget = domHost && domHost.$ ? domHost.$[scrollTarget] : polymer_dom_js.dom(this.ownerDocument).querySelector('#' + scrollTarget);
...
`,
  ...
  behaviors: [templatizerBehavior_js.Templatizer, IronResizableBehavior, IronScrollTargetBehavior, mutableDataBehavior_js.OptionalMutableDataBehavior],
   ...
   */
  get _itemsParent() {
    return polymer_dom_js.dom(polymer_dom_js.dom(this._userTemplate).parentNode);
  },
  ...
  /** @override */
  attached: function () {
    this._debounce('_render', this._render, async_js.animationFrame); // `iron-resize` is fired when the list is attached if the event is added
  ...
  _setOverflow: function (scrollTarget) {
    ...
    this._debounce('_render', this._render, async_js.animationFrame);
  },
  ...
  _scrollHandler: function () {
    ...
    if (Math.abs(delta) > this._physicalSize && this._physicalSize > 0) {
      ...
      this._debounce('_increasePoolIfNeeded', this._increasePoolIfNeeded.bind(this, 0), async_js.microTask);
    }
  },
  ...
  _increasePoolIfNeeded: function (count) {
    ...
    if (this._virtualEnd >= this._virtualCount - 1 || nextIncrease === 0) ; else if (!this._isClientFull()) {
      this._debounce('_increasePoolIfNeeded', this._increasePoolIfNeeded.bind(this, nextIncrease), async_js.microTask);
    } else if (this._physicalSize < this._optPhysicalSize) {
      // Yield and increase the pool during idle time until the physical size is
      // optimal.
      this._debounce('_increasePoolIfNeeded', this._increasePoolIfNeeded.bind(this, this._clamp(Math.round(50 / this._templateCost), 1, nextIncrease)), async_js.idlePeriod);
    }
  },
  ...
  scrollToIndex: function (idx) {
    ...
    flush_js.flush(); // Items should have been rendered prior scrolling to an index.
  ...
  _selectionHandler: function (e) {
    ...
    var target = polymer_dom_js.dom(e).path[0];
  ...
  _focusPhysicalItem: function (idx) {
    ...
    if (!focusable) {
      focusable = polymer_dom_js.dom(physicalItem).querySelector('[tabindex="' + SECRET_TABINDEX + '"]');
  ...
  _notifyInstancePropV2: function (inst, prop, value) {
    if (path_js.matches(this.as, prop)) {
      ...
      this.notifyPath(path_js.translate(this.as, 'items.' + idx, prop), value);
    }
  },
  ...
  _getActiveElement: function () {
    ...
    return polymer_dom_js.dom(itemsHost ? itemsHost.root : document).activeElement;
  }
});
...
`,
   ...
   */
  get visible() {
    polymerLegacy_js.Base._warn('`visible` is deprecated, use `opened` instead');
  ...
  show: function (properties) {
    ...
    for (var property in properties) {
      if (property.indexOf('_') === 0) {
        polymerLegacy_js.Base._warn('The property "' + property + '" is private and was not set.');
      } else if (property in this) {
        this[property] = properties[property];
      } else {
        polymerLegacy_js.Base._warn('The property "' + property + '" is not valid.');
      }
    }
...
`,
  ...
  _setKeyboardFocusTrap: function () {
    ...
    var focusableElements = polymer_dom_js.dom(this).querySelectorAll(focusableElementsSelector);
  ...
  _tabKeydownHandler: function (event) {
    ...
    if (this._drawerState === this._DRAWER_STATE.OPENED && event.keyCode === TAB_KEYCODE) {
      if (event.shiftKey) {
        if (this._firstTabStop && polymer_dom_js.dom(event).localTarget === this._firstTabStop) {
          event.preventDefault();

          this._lastTabStop.focus();
        }
      } else {
        if (this._lastTabStop && polymer_dom_js.dom(event).localTarget === this._lastTabStop) {
          event.preventDefault();
...
const AppLayoutBehavior = [IronResizableBehavior, {
  ...
  _appResetLayoutHandler: function (e) {
    if (polymer_dom_js.dom(e).path[0] === this) {
      return;
    }
  ...
  resetLayout: function () {
    ...
    this._layoutDebouncer = debounce_js.Debouncer.debounce(this._layoutDebouncer, async_js.animationFrame, cb);
    flush_js.enqueueDebouncer(this._layoutDebouncer);
...
`,
   ...
   */
  get drawer() {
    return polymer_dom_js.dom(this.$.drawerSlot).getDistributedNodes()[0];
  },
  ...
  _clickHandler: function (e) {
    var target = polymer_dom_js.dom(e).localTarget;
...
`,
  ...
  get _stickyEl() {
    ...
    var nodes = polymer_dom_js.dom(this.$.slot).getDistributedNodes(); // Get the element with the sticky attribute on it or the first element in
    // the light DOM.
  ...
  _ensureBgContainers: function () {
    if (!this._bgContainer) {
      ...
      polymer_dom_js.dom(this.root).insertBefore(this._bgContainer, this.$.contentContainer);
    }
  },
  _getDOMRef: function (id) {
    switch (id) {
      ...
      case 'mainTitle':
        return polymer_dom_js.dom(this).querySelector('[main-title]');

      case 'condensedTitle':
        return polymer_dom_js.dom(this).querySelector('[condensed-title]');
    }
...
`,
   ...
   */
  get header() {
    return polymer_dom_js.dom(this.$.headerSlot).getDistributedNodes()[0];
  },
...
found at http://polymer.github.io/PATENTS.txt
*/
const template$9 = htmlTag_js.html`
<custom-style>
  <style is="custom-style">
    html {
...
`,
  ...
  get _propertyForValue() {
    return caseMap_js.dashToCamelCase(this.attrForValue);
  },

  get _inputElement() {
    return polymer_dom_js.dom(this).querySelector(this._inputSelector);
  },
...
const PaperInputBehaviorImpl = {
  ...
  attached: function () {
    ...
    if (!polymerElement_js.PolymerElement && this.inputElement && this._typesThatHaveText.indexOf(this.inputElement.type) !== -1) {
  ...
  _updateAriaLabelledBy: function () {
    var label = polymer_dom_js.dom(this.root).querySelector('label');
